...
| `maskMetaKeys`          | `string[]`                                                         | `[]`                             | Keys whose values are replaced with `[REDACTED]` before serialization, matched **case-insensitively** and applied **deeply**, in BOTH file and console pipelines and in both formats. Covers metadata, object and array messages (`logger.info({ user, password })`), non-string stacks, and `Error`s inside metadata, including their `cause` chain. Empty by default; without a mask, output is unchanged. See [Masking with `maskMetaKeys`](#masking-with-maskmetakeys) for what is covered, how failures render, and the `toJSON()` boundary.             |
...
- **Metadata:** the object passed after the message (`logger.info("Login", meta)`), and the extra keys of a single object that has a `message`. The walk is deep: nested objects, arrays, and the enumerable own fields of class instances.
...
- A nested value's own `toJSON()` is not called by the masking walk (see the boundary below); if it throws, the line falls back to the serializer's own failure rendering.

**Redaction boundary:** nested values that define their own `toJSON()` (`Date`, `URL`, custom serializable classes, an HTTP client error that defines one) or have no enumerable own keys (`Map`, `Set`, `RegExp`) are serialized through that method or built-in and are **not** key-redacted. That includes such a value inside an error's `cause` or `errors`, and an `Error` inside such a `toJSON()` output is rendered without masking. An `Error` with its own `toJSON()` is rendered through that method, so fields it leaves out stay out. Log only the fields you need from such values, or normalize them to plain objects first.
...
| `maskBodyKeys`           | `string[]`                                                               | `[]`                                  | Keys replaced with `[REDACTED]`. Applies deeply and case-insensitively, including arrays, the enumerable own fields of class instances, and `Error`s (their own fields, `cause` chain and `AggregateError` members, keeping `name`, `message` and `stack`). Values with a custom `toJSON()` (`Date`, `URL`) or no enumerable keys (`Map`, `Set`) pass through un-redacted, also inside an error's `cause` — use `redactPaths` for those.                                                                                   |
...
- `maskMetaKeys` redaction continues to apply, walking nested objects, arrays and errors before `JSON.stringify`. Secret values are replaced with the literal string `"[REDACTED]"` before the line is serialized.
...
- With `maskMetaKeys`, masked keys inside an error, its `cause` chain and `AggregateError` members are redacted; see [Masking with `maskMetaKeys`](#masking-with-maskmetakeys) for the `toJSON()` boundary.
