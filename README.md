...
- A `cause` that points back at its own error, set non-enumerably as the `new Error(msg, { cause })` option creates it, renders as `"[Circular]"` in JSON mode. In pretty format that error still renders as `{}`, with its sibling fields intact, as in earlier versions. An error that references itself through an enumerable property (`err.self = err`, or `err.cause = err` on an error created without the `cause` option) renders `"[Circular]"` in JSON mode, while the pretty metadata block renders `[UNSERIALIZABLE]`, as before. With `maskMetaKeys` set, a self-referencing `cause` renders the error's fields and `"[Circular]"` in both formats.
