...
| `colorize`              | `boolean \| { message?: boolean; level?: boolean; all?: boolean }` | `{ level: true, message: true }` | Controls ANSI colorization of the console transport. `false` disables colorization entirely; `true` colors both the `[LEVEL]` token and the message body; an object honors `level` / `message` flags independently, with `all: true` overriding both. The message is colored after it is rendered, so the console shows the same message text as the file line (an `undefined` message, a BigInt, and full-depth JSON for objects and arrays included). File transports are never colorized.                                                                  |
| `maskMetaKeys`          | `string[]`                                                         | `[]`                             | Keys whose values are replaced with `[REDACTED]` before serialization, matched **case-insensitively** and applied **deeply**, in BOTH file and console pipelines and in both formats. Covers metadata, object and array messages (`logger.info({ user, password })`), non-string stacks, and `Error`s inside metadata, including their `cause` chain. Empty by default; without a mask, output is unchanged. See [Masking with `maskMetaKeys`](#masking-with-maskmetakeys) for what is covered, how failures render, and what masking cannot reach.           |
...
`level` is the logger's initial level. Assigning `logger.level` later takes effect immediately on the console, the module file, and the global file, because those built-in transports carry no level of their own and inherit the logger's current level on every write. For every level in the hierarchy, `logger.isLevelEnabled()` gives the same answer as what is emitted:

```ts
const logger = createLogger({ moduleName: "api" });
logger.debug("dropped"); // default level "info"

logger.level = "debug";
logger.isLevelEnabled("debug"); // true
logger.debug("written to the console, api-<date>.log, and all-logs-<date>.log");
```

- Passing `consoleLevel` pins the console to that level, even when it equals `level`: runtime changes then reach the files but not the console.
- Loggers that share the global file keep independent levels; raising one logger's level does not change what the others write.
- Do not remove a built-in transport and add it back (`logger.remove(t)` / `logger.add(t)`, or `logger.configure({ transports })` reusing it). Winston closes a transport when it is removed. For the module file that ends its stream, so a re-added one writes nothing and the next write can surface as an uncaught `write after end` error. For the global file it releases the logger's share, so a re-added handle keeps writing only while another logger still holds the file, and its lines are lost once the last one shuts down. The Console survives removal, but winston links a transport to its logger only on the first pipe, so a re-added Console without its own level no longer sees the logger's level and accepts every entry; set `t.level` (or pass `consoleLevel`) before re-adding it. To change destinations, create a new logger instead.
- `logger.transports[i].level` reads `undefined` for the built-in module-file and global-file transports, and for the console unless `consoleLevel` was passed (then it reads that value). An `additionalTransports` entry created with its own `level` keeps it.
- Winston does not validate the assigned value, and invalid values behave in opposite ways. A non-empty string outside the hierarchy, such as `"silent"`, matches no level, so every inheriting transport drops every entry, errors and crash records included. An empty or missing value (`""`, `undefined`, `null`, typically an unset environment variable such as `logger.level = process.env.LOG_LEVEL`) leaves the transports with no level, so they write **every** entry, `debug` and `silly` included, while `isLevelEnabled()` reports `false`. Validate a configured level before assigning it, and use `logger.silent = true` to silence a logger.

### Masking with `maskMetaKeys`

`maskMetaKeys: ["password"]` replaces the value of every `password` key with `"[REDACTED]"` before the line is serialized. Keys are matched case-insensitively, and the redaction runs in the file and console pipelines of both formats. The option is empty by default, and without a mask the output is unchanged.

```ts
const logger = createLogger({ maskMetaKeys: ["password"] });

logger.info("Login", { user: "bob", password: "topsecret" }); // "password": "[REDACTED]"
logger.info({ user: "bob", password: "topsecret" }); // masked too (object message)
```

What is masked:

- **Metadata:** the object passed after the message (`logger.info("Login", meta)`), and the extra keys of a single object that has a `message`. The walk is deep: nested objects, arrays, the enumerable own fields of class instances, and the output of nested `toJSON()` methods (below).
- **Object and array messages:** winston puts a single-argument payload without a truthy `message` into the message slot, so `logger.info({ user, password })`, `logger.info([{ password }])`, `logger.info({ message: "", password })` and `logger.info({ message: { password } })` are masked as well. If the message value has a `toJSON()` method, it is called first and its result is masked by its own keys, the way the serializer reads it, so a DTO without a `message` field whose `toJSON()` returns the secret is masked too.
- **A metadata object's own `toJSON()`:** in the default pretty format, a metadata object with its own `toJSON()` method is serialized through that method, so its output is masked by its own keys. The method runs on the masked copy, so a field it reads through `this` stays masked. JSON mode also masks the `toJSON()` output of the logged entry by its own keys, but calls the method on the original object, so a masked field that the method copies from `this` under another key name (`toJSON() { return { pw: this.password }; }`) is not masked there.
- **Non-string stacks:** an object or array `stack` is masked the same way. String and BigInt messages and string stacks, including every real `Error` stack, are never changed.
- **Errors inside metadata:** an `Error` keeps its `name`, `message` and `stack` when one of its fields is masked. The walk also reads its `cause` chain (the `new Error(msg, { cause })` form) and an `AggregateError`'s `errors`, so a masked key inside them is redacted before anything prints it. Errors created in another realm (a `vm` context) are covered.
- **Nested values with their own `toJSON()`:** a DTO, a database document, an array, or an HTTP client error that defines `toJSON()` (for example one whose output includes its request headers), nested anywhere, including inside an error's `cause` or an `AggregateError`, is printed through that method, so the masking walk calls it the same way and masks its output by its own keys. A class instance's `toJSON()` runs on the instance, so `#private` fields work; a plain object's own `toJSON()` runs on the masked copy, so a field it reads through `this` is already masked. A `toJSON()` that returns a string (a `Date`, a `URL`, a `moment`) renders unchanged. With a mask set, such a method runs twice per line (in the walk and again in the serializer), and so does one whose output the walk leaves as it is (a class instance or an error with nothing to mask, a `Buffer`); one that returns a plain object or an array runs once. Without a mask the walk never calls `toJSON()`.

With a mask set, a message is walked like metadata: a circular reference renders as `"[Circular]"`, own `__proto__` / `constructor` / `prototype` keys are dropped, and nesting deeper than 256 levels renders as `"[MaxDepth]"`.

**Masking fails closed.** When a read throws during masking, the value is replaced, never written raw:

- A message or stack whose walk throws (a throwing getter or `toJSON()`) renders as the string `"[RedactionFailed]"`, and the log call does not throw.
- A metadata value whose getter throws renders as `"[RedactionFailed]"` in JSON mode. In the pretty file the metadata block is replaced by `{ "_redactionFailed": true }`. When the getter is a top-level metadata key, the pretty console instead renders the other fields, masked, without that one; a getter that throws deeper inside a value fails the console block closed the same way.
- If the logged entry's `toJSON()` throws, the JSON line keeps only `level`, `message` and `_redactionFailed: true`. In pretty mode an own `toJSON` property that throws renders the metadata block as `[UNSERIALIZABLE]`; a `toJSON()` inherited from a class is never called in pretty mode, so the instance's own fields are rendered, masked by key.
- Inside an `Error`, only the affected field is replaced, and a getter on the error itself that throws is left out.
- A nested value whose `toJSON()` throws, or whose `toJSON` property is a getter that throws, renders as `"[RedactionFailed]"`, and the rest of the line still renders.

**What masking cannot reach:** masking matches keys, never text. A string is printed as it is, so an error's `message` and `stack` print exactly as the error builds them (an error class that writes its own fields into its message, such as a validation error that lists the rejected input, prints them even under a mask), and a class instance's `toJSON()`, which runs on the instance, prints a value it copies from a masked field under another key name or returns as its whole result. Values with no enumerable own keys (`Map`, `Set`, `RegExp`) and binary values (a `Buffer`'s `type` and `data`, typed arrays) render as the serializer renders them, and a function that carries its own `toJSON()` is printed through it without being masked in pretty format and in the request middleware's body preview (JSON mode drops a function). A nested plain object's own `toJSON()` runs on the masked copy, where a nested value whose `toJSON()` returns a plain object or an array is already replaced by that masked output, so a `toJSON()` that calls a method on such a value fails and renders `"[RedactionFailed]"`. A mask that names `toJSON` itself replaces a plain object's own method, so that object's own fields print. An `Error` with its own `toJSON()` is rendered through that method, so fields it leaves out stay out. Log only the fields you need from such values, or normalize them to plain objects first.

**The objects you log are not rewritten.** Masking, `Error` flattening, the `module` field and the timestamp all work on copies, so a plain object you log keeps its original values afterwards and stays safe to persist or return. For example, `logger.error({ message: err, requestId })` leaves `message` the same `Error`, with no `stack` or error fields added. Apart from winston's internal Symbol slots, these are the exceptions:

- winston itself sets `level` (and any `logger.defaultMeta` keys) on an object it uses as the log entry (`logger.log("info", obj)`, `logger.log(obj)`, or `logger.info(obj)` when `obj` has a truthy `message`; without one, winston logs `{ message: obj }` instead), before any format runs, exactly as bare winston does.
- An array or class instance that is the log entry gets a `timestamp` property written onto it, because a plain copy would lose its array shape or its prototype and `toJSON()`.
- A class instance whose `message` is an `Error` is still flattened in place by winston's `errors()` format: its `message` becomes the error's message string, and the error's `stack` and own fields are copied onto it. A plain copy would lose its `toJSON()`, so JSON mode would print the fields that method leaves out.
- A Proxy whose `message` exists only through its `get` trap is flattened in place the same way, through its traps, because a copy cannot reproduce a property the object does not own.
...
- `maskMetaKeys` redaction continues to apply, walking nested objects, arrays, errors and the output of nested `toJSON()` methods before `JSON.stringify`. Secret values are replaced with the literal string `"[REDACTED]"` before the line is serialized.
- An `Error` inside metadata is written as an object with its `name`, `message`, `stack`, own fields, `cause` and `errors` (see [Errors in log lines](#errors-in-log-lines)).
...
- **Shared transports and lifetime:** `shutdownLogger(child)`, `child.close()` and `child.end()` act on the root. `shutdownLogger(child)` and `shutdownLogger(root)` return the same promise, and both `close()` and `end()` drop the root from the cache so the next `createLogger()` builds a fresh logger; `close()` also leaves crash capture and releases the shared global file. `child.end(entry)` writes `entry` with the child's metadata before ending the root. `child.add()`, `remove()`, `clear()`, `configure()`, `pipe()` and `unpipe()` change the root's transports, exactly as calling them on the root does. Every method that returns the logger (the level methods, `log()`, `close()`, `end()`, `add()`, `remove()`, `clear()`, `unpipe()`, `on()` and the other event-emitter methods, and the unknown-method fallback) returns the logger it was called on, so a chain such as `child.info("a").success("b")` or `logger.info("a").child(meta)` stays inside the package's wrapper. A child of a logger that was shut down stays shut down; derive children again from the new logger.
...
The record is written at `error` through the elected logger's transports, so each of them gates it like any other entry: it survives any level in the hierarchy, but after `logger.level = "silent"` on the elected logger only a console pinned with `consoleLevel` (or an `additionalTransports` entry with its own level) still records it (see [Changing the level at runtime](#changing-the-level-at-runtime)).
...
**A logged `Error`** (`logger.error(err)`) renders its message and its `stack`, and its own enumerable fields (such as `code`) are rendered as metadata. winston's `errors()` format copies only those, so a `cause` set through the `new Error(msg, { cause })` option (a non-enumerable property) is not rendered, while one assigned afterwards on an error created without that option (`err.cause = inner`, an enumerable property) is rendered as metadata. To log the chain either way, pass the error as metadata: `logger.error("Payment failed", { err })`.

**An `Error` inside metadata** renders as an object with `name`, `message`, `stack`, its own enumerable fields, its `cause` (rendered the same way, down the chain) and, for an `AggregateError`, its `errors`. Earlier versions wrote only the error's own enumerable fields, so a plain error rendered as `"err": {}`. An error from another realm (a `vm` context) is covered, and an error with its own `toJSON()` renders through that method.

```ts
const logger = createLogger({ moduleName: "payments" });
const cause = new Error("insufficient funds");
logger.error("Payment failed", { err: new Error("card declined", { cause }), orderId: 7 });
```

The pretty file line lists the fields in the order above:

```text
UTC: 2026-05-04 14:30:22
[ERROR] (payments)
Payment failed
{
  "err": {
...
With `format: "json"`, keys are sorted as always:

```text
{"err":{"cause":{"message":"insufficient funds","name":"Error","stack":"Error: insufficient funds\n    at …"},"message":"card declined","name":"Error","stack":"Error: card declined\n    at …"},"level":"error","message":"Payment failed","module":"payments","orderId":7,"timestamp":"2026-05-04 14:30:22"}
```

- A `cause` that points back at its own error, set non-enumerably as the `new Error(msg, { cause })` option creates it, renders as `"[Circular]"` in JSON mode. In pretty format the whole metadata block is rendered as in earlier versions: that error, and any other error in the same block, renders as `{}`, and the other fields stay intact. An error that references itself through an enumerable property (`err.self = err`, or `err.cause = err` on an error created without the `cause` option) renders `"[Circular]"` in JSON mode, while the pretty metadata block renders `[UNSERIALIZABLE]`, as before. With `maskMetaKeys` set, a self-referencing `cause` renders the error's fields and `"[Circular]"` in both formats.
- The `error` field of a [crash record](#crash-capture) renders the same way, and so do errors in the request middleware's `enrich()` context and request body (a `redactPaths` entry such as `context.err.stack` can target their fields).
- With `maskMetaKeys`, masked keys inside an error, its `cause` chain and `AggregateError` members are redacted, including inside the output of a `toJSON()` method there; see [Masking with `maskMetaKeys`](#masking-with-maskmetakeys) for what masking cannot reach.
...
- **URL query redaction (`maskQueryKeys`)**: edits the logged `req.originalUrl` / `req.url` query string **in place** — it locates each `key=value` pair in the raw query and replaces only a matched parameter's value with the literal `[REDACTED]`, leaving every other byte untouched. Sibling-parameter encoding (`%20`, `%5B`/`%5D`, `%2B`), the `//host` authority of protocol-relative URLs, the scheme/host of absolute URLs, and the URL fragment are all preserved exactly; parameters are never re-ordered or re-encoded, and the function never throws. Handles both absolute and relative URLs, including a URL whose only `?` appears inside the fragment (e.g. a hash-router path like `"/dash#/r?token=abc"`), which has no real query component and is returned unchanged. Default mask list (opt-out by passing `false`):
  ...
  - `secret`, `password`
- **Surgical path redaction (`redactPaths`)**: opt-in array of dot-notation paths into the resolved entry (e.g. `["body.user.password"]`). The leading `body` segment is rewritten to `requestBody` to match the public field name. Missing intermediate keys are a graceful no-op — we never create new sub-paths just to write `[REDACTED]`. Body-scoped paths (`body.*` / `requestBody.*`) are applied to the object graph BEFORE the `maxBodyLength` truncation decision, so a secret targeted only by `redactPaths` (with no corresponding `maskBodyKeys` entry) cannot survive inside the `_preview` field of an over-limit body's truncation envelope. The request body, the `enrich()` context, and any captured request/response headers are all redacted on internal owned copies — even when the body or context is a class instance that defines its own `toJSON()` (a Mongoose document, a DTO, `req.user`) — and a path only ever steps through and writes into those package-owned plain objects and arrays, following their own data properties (never an inherited property, and never a getter, which is not run). So your original `req.body` / `req.session` / `req.user` / `req.headers` objects (and the live response-header values behind `res.getHeaders()`) are never mutated by a `redactPaths` write, and no path can reach `Object.prototype`: `__proto__`, `constructor` and `prototype` segments are refused. A header value the entry keeps by reference (a `Date`, a class instance with its own `toJSON()`, a `Buffer`, a `Map`) ends the path, because everything behind it is your application's state; to hide such a header, list it in `maskHeaderKeys`. A multi-segment path starts at `body` / `requestBody`, `requestHeaders`, `responseHeaders` or `context`; a single segment such as `url` redacts that field. The list is copied when the middleware is created, so changing your array afterwards has no effect. Path redaction is also best-effort and never throws: a path that targets an array's `length`, a frozen or getter-only property, or any other slot it cannot safely write is skipped rather than allowed to drop the entire log entry.
...
- **Unreadable payloads**: a payload whose keys or prototype cannot be read (a Proxy whose `ownKeys` or `getPrototypeOf` trap throws), logged as the entry itself (`logger.info(proxy)` when it has a truthy `message`, or `logger.log("info", proxy)`), does not throw out of the log call. The line is written with the level, the message when it is a string (otherwise `[UNSERIALIZABLE]`) and `_unserializable: true`; the rest of the payload is dropped. This does not hold through a child logger, whose entry copy runs inside winston before any of this package's formats (see [Child Loggers](#child-loggers)). A logged `Error` with an own enumerable getter that throws is written as a degraded line marked `_errorFlattenFailed: true` instead of throwing.
